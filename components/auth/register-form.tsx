...
export function RegisterForm() {
  ...
  function validate(formData: FormData): string | null {
    ...
    if (username.length < MIN_NAME_LENGTH) return 'El nombre de usuario es muy corto'
    if (!email) return 'El correo electrónico es obligatorio'
    if (!EMAIL_REGEX.test(email)) return 'Introduce un correo electrónico válido'
    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="flex flex-col items-center pb-10 pt-4">
        <p className="text-sm text-muted-foreground">
          ...
          <Link href="/auth/login" className="font-semibold italic text-orange-primary hover:underline">
            Inicia Sesion
          </Link>
        </p>
      </div>
